# -*- coding: utf-8 -*-
"""Assigment2_guide.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/github/omid-reza/COMP-6781/blob/main/Assignment2/Assigment2_guide.ipynb
...
def word_to_index(vocabulary):
  ...
  for index, word in enumerate(vocabulary.keys(), start=1):
...
def generate_dataset(data, window_size,word_to_index):
  ...
  for i in range(window_size,len(data)-window_size):
    surrounding = data[i-window_size:i] + data[i+1:i+window_size+1] #get surrounding words based on window size
    target = data[i] #get target word (middle word)
    #append to surrounding
    surroundings.append([word_to_index.get(word, word_to_index['OOV']) for word in surrounding])
    #append to targets
    targets.append(word_to_index.get(target, word_to_index['OOV']))
...
def get_k_nearest_words(k, word, vocabulary,model,word_to_index):
  ...
  word_embedding = word if isinstance(word, torch.Tensor) else get_embedding(word, model, word_to_index)
